...
instance (GaloisField k, EPCurve e k) => Curve 'Edwards 'Projective e k where
...
instance (GaloisField k, EPCurve e k) => Group (EPPoint e k) where
...
instance (GaloisField k, EPCurve e k) => Arbitrary (EPPoint e k) where
  arbitrary = suchThatMap arbitrary pointX

-- Edwards projective points are equatable.
instance (GaloisField k, EPCurve e k) => Eq (EPPoint e k) where
  p == p' = case (dehom p, dehom p') of
    (P x y z, P x' y' z') -> x == x' && y == y' && z == z'

-- Edwards projective points are pretty.
instance (GaloisField k, EPCurve e k) => Pretty (EPPoint e k) where
  pretty (P x y z) = pretty (x, y, z)

-- Edwards projective points are random.
instance (GaloisField k, EPCurve e k) => Random (EPPoint e k) where
  random g = case pointX x of
    Just p -> (p, g')
    _      -> random g'
    where
...
-- | Dehomogenisation of Edwards projective points.
dehom :: (GaloisField k, EPCurve e k) => EPPoint e k -> EPPoint e k
